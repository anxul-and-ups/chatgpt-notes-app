...
class NoteRepository(private val noteDao: NoteDao) {
    ...
    suspend fun moveFolderNotes(oldFolder: String, newFolder: String) =
        noteDao.moveFolderNotes(oldFolder, newFolder)

    suspend fun hideNote(note: NoteEntity) =
        noteDao.updateHidden(note.id, true, note.folder)

    suspend fun unhideNote(note: NoteEntity) =
        noteDao.updateHidden(note.id, false, if (note.originalFolder.isBlank()) "All Notes" else note.originalFolder)
}
