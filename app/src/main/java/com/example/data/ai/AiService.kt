...
class AiService {
    ...
    suspend fun generateResponse(
        ...
        try {
            val cleanKey = apiKey.trim()
            val cleanModel = if (model.isBlank()) "gemini-2.0-flash" else model
            val selectedProvider = provider.trim().ifBlank { "Gemini" }
            val contentsArray = JSONArray()
            ...
            contentsArray.put(JSONObject().apply {
                ...
                for (imgBase64 in attachmentImagesBase64.take(6)) {
                    parts.put(JSONObject().put("inline_data", JSONObject().apply {
                        put("mime_type", "image/png"); put("data", imgBase64)
                    }))
                }
                put("parts", parts)
            })
            ...
            when {
                selectedProvider.equals("Gemini", true) -> {
                    val url = (if (baseUrl.isBlank()) "https://generativelanguage.googleapis.com/v1beta/models/$cleanModel:generateContent" else baseUrl.trimEnd('/')) + "?key=$cleanKey"
                    jsonBody = JSONObject().put("contents", contentsArray)
                    request = Request.Builder().url(url).post(jsonBody.toString().toRequestBody("application/json".toMediaType())).build()
                }
                selectedProvider.equals("Anthropic", true) -> {
                    val url = if (baseUrl.isBlank()) "https://api.anthropic.com/v1/messages" else baseUrl.trimEnd('/')
                    val messages = JSONArray()
                    for (turn in history.takeLast(8)) messages.put(JSONObject().put("role", if (turn.role == "bot") "assistant" else "user").put("content", turn.content))
                    messages.put(JSONObject().put("role", "user").put("content", latestUserMessage))
                    jsonBody = JSONObject().put("model", cleanModel).put("max_tokens", 4096).put("system", systemInstruction).put("messages", messages)
                    request = Request.Builder().url(url).addHeader("x-api-key", cleanKey).addHeader("anthropic-version", "2023-06-01").post(jsonBody.toString().toRequestBody("application/json".toMediaType())).build()
                }
                else -> {
                    val url = if (baseUrl.isBlank()) when {
                        selectedProvider.equals("DeepSeek", true) -> "https://api.deepseek.com/chat/completions"
                        selectedProvider.equals("Kimi", true) -> "https://api.moonshot.cn/v1/chat/completions"
                        selectedProvider.equals("OpenCode", true) -> "https://opencode.ai/zen/v1/chat/completions"
                        selectedProvider.equals("Hugging Face", true) -> "https://router.huggingface.co/v1/chat/completions"
                        else -> "https://api.openai.com/v1/chat/completions"
                    } else baseUrl.trimEnd('/')
                    val messages = JSONArray().put(JSONObject().put("role", "system").put("content", systemInstruction))
                    for (turn in history.takeLast(8)) messages.put(JSONObject().put("role", if (turn.role == "bot") "assistant" else "user").put("content", turn.content))
                    messages.put(JSONObject().put("role", "user").put("content", latestUserMessage))
                    jsonBody = JSONObject().put("model", cleanModel).put("messages", messages)
                    request = Request.Builder().url(url).addHeader("Authorization", "Bearer $cleanKey").post(jsonBody.toString().toRequestBody("application/json".toMediaType())).build()
                }
            }
            ...
            if (response.isSuccessful && !responseBody.isNullOrBlank()) {
                val jsonResponse = JSONObject(responseBody)
                if (selectedProvider.equals("Gemini", true)) {
                    val candidates = jsonResponse.optJSONArray("candidates")
                    if (candidates != null && candidates.length() > 0) {
                        val parts = candidates.getJSONObject(0).getJSONObject("content").getJSONArray("parts")
                        if (parts.length() > 0) return@withContext parts.getJSONObject(0).optString("text")
                    }
                } else if (selectedProvider.equals("Anthropic", true)) {
                    val content = jsonResponse.optJSONArray("content")
                    if (content != null && content.length() > 0) return@withContext content.getJSONObject(0).optString("text")
                } else {
                    return@withContext jsonResponse.optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("message")?.optString("content")?.takeIf { !it.isNullOrBlank() } ?: generateSmartFallback(prompt, noteContext, attachmentContent)
                }
            }
        } catch (_: Exception) {
